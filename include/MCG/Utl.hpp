...
/// \date 2007/11/19
/// \author Peter Dodds
/// \brief CGM::Utl function declarations
...
#ifndef _CGM_Utl_hpp_
#define _CGM_Utl_hpp_
...
namespace CGM
{
  ...
  public:
    ...
    /// \details The line is defined by two points on it, \a lineA and \a lineB. This performs two 
    /// calls to Vector3::MagnitudeSquared(), which do not call CGM_SQRT().
    /// \param point - point to test against line
    /// \param lineA - first point on line
    /// \param lineB - second point on line
    /// \returns CGM::Real - shortest distance squared
    /// \see DistanceToLine()
    ...
    /// \details The line is defined by two points on it, \a lineA and \a lineB. This performs two 
    /// calls to Vector3::MagnitudeSquared(), which do not call CGM_SQRT(). If the closest point to the 
    ...
    // -----------------------------------
    /// \brief Get difference in rotation between two CGM::Quaternions
    /// \details The returns the value of w for the rotation between \c A and \c B.
    /// \param A - first CGM::Quaternion
    /// \param B - second CGM::Quaternion
    /// \returns CGM::Real - w value of CGM::Quaternion between \c A and \c B
    static Real EstimateRotationBetween (const Quaternion& A, const Quaternion& B);

    // -----------------------------------
    /// \brief Get angle between two CGM::Quaternions
    /// \details The returns the angle of the rotation between \c A and \c B.
    /// \param A - first CGM::Quaternion
    /// \param B - second CGM::Quaternion
    /// \returns CGM::Real - angle between \c A and \c B
    ...
    /// \returns CGM::Real - length of the arc
