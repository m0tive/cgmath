...
/// \date 2007/12/07
/// \author Peter Dodds
/// \brief CGM::Quaternion function definitions
...
namespace CGM
{
  ...
  {
    CGM::Real _angle = angle * 0.5;
    CGM::Real sin_a = CGM_SIN(_angle);
    CGM::Real cos_a = CGM_COS(_angle);
    //sin_a = 1/sin_a;
  ...
  //------------------------------------------------ Quaternion::ToMatrix
  CGM::Matrix Quaternion::ToMatrix() const
  {
  ...
  void Quaternion::ToAxisAngles( Real& angle, Real& _x, Real& _y, Real& _z ) const
  {
    ...
    angle = CGM_ACOS(quat.w)*2;
    Real sina = CGM_SQRT(1.0 - quat.w*quat.w);
    if (fabs(sina) < 0.0005)
    {
      // sina = 1;
      _x = quat.x;
  ...
  CGM::EulerAngle Quaternion::ToEulerAngles() const
  {
    return EulerAngle (*this);
#if 0
    _x = GetRollAngle();
    _y = CGM_ASIN((2*(w*y - z*x)));
    _z = GetYawAngle();
#endif // 0
  }

  //------------------------------------------------ Quaternion::GetAngle
  CGM::Real Quaternion::GetAngle()
  {
    return CGM_ACOS(w)*2;
  }
  ...
  Real Quaternion::GetRollAngle () const
  {
    ...
    if (test > 0.4999)
      ...
      a = CGM_ATAN2((2*(w*x + y*z)) , (1 - 2*(x*x + y*y)));
    //Real a = CGM_ATAN2(2*(q1.x*q1.w-y*z) , 1 - 2*sqx - 2*sqz);
    return a;
  }
  ...
  Real Quaternion::GetYawAngle () const
  {
    ...
    if (test > 0.4999 || test < -0.4999)
      ...
      a = CGM_ATAN2((2*(w*z + x*y)) , (1 - 2*(y*y + z*z)));
    //Real a = CGM_ATAN2(2*(y*w - x*z) , 1 - 2*(y*y - z*z));
    return a;
  }
#endif // 0
  ...
  CGM::Quaternion Quaternion::Normalised() const
  {
    double tmp = x*x + y*y + z*z + w*w;
    if(tmp!=0.0)
      tmp = 1.0 / sqrt(tmp);
  ...
  CGM::Real Quaternion::MagnitudeSquared() const
  {
    return x*x+y*y+z*z+w*w;
  }

  //----------------------------------------------------------- Quaternion::Mag
  CGM::Real Quaternion::Magnitude() const
  {
    return CGM_SQRT(x*x+y*y+z*z+w*w);
  }
  ...
  //----------------------------------------------------------- Quaternion::operator-
  CGM::Quaternion Quaternion::operator-() const
  {
    return Quaternion (-x,-y,-z,w);
  }

  //----------------------------------------------------------- Quaternion::operator*
  CGM::Quaternion Quaternion::operator*( const Quaternion& q ) const
  {
  ...
  CGM::Quaternion Quaternion::operator* (const Real& r) const
  {
  ...
  Quaternion SLERP (const Quaternion& q1, const Quaternion& q2, const Real& t)
  {
    ...
    Real theta = CGM_ACOS(prod);
    Real sinTheta = CGM_SIN(theta);

    if (sinTheta != 0.0f)
    {
      iterp_inv = CGM_SIN(iterp_inv * theta) / sinTheta;
      iterp = CGM_SIN(iterp * theta) / sinTheta;
    }
